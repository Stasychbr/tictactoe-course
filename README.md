...
Вход: `playable_part` - часть клеток, доступная для игры; `max_obstacle_len` - максимальная длина препятствия; `gap` - зазор между отдельными препятствиями.

Выход: поле, случайным образом заполненное препятствиями.

1. Сгенерировать случайное число `obstacle_len` от 0 до `max_obstacle_len`. Сгенерировать случайный набор шагов (вверх, вниз, влево, вправо) с учетом невозможности ходить в обратную сторону длиной `obstacle_len`.

2. Если поле _достаточно пустое_, то сгенерировать случайную позицию в пределах поля, начиная с которой предполагается искать координаты для расположения препятствия. Если до этого уже была сложность с размещением препятствия, то поле помечается как _достаточно заполненное_, и поиск позиции для препятствия начинается с клетки (0, 0).

3. Начиная с выбранной позиции перебрать все клетки поля и попробовать вставить в них препятствие. Препятствие размещается с учетом нижней границы числа игровых клеток `M * N * playable_part`. Лишняя часть препятствия обрубается.

4. Если получилось вставить препятствие, то пометить клетки вокруг него в радиусе `gap` как недоступные для размещения других препятствий. Если не получилось найти позицию для препятствия, то повторить шаг 1, но при этом если шаг 4 не завершился удачей `sqrt(M * N)` раз подряд, то остановить процесс генерации. Иначе перейти к шагу 5.

5. Если игровых клеток на поле стало `M * N * playable_part`, то остановить процесс генерации. Если свободных клеток больше, перейти к шагу 1.
